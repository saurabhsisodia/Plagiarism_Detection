...
class Plagirism_Detection(object):
	...
	def All_Hash(self):
		...
		for h in Hash_set1:
			if checked1[h]==False:
				SH+=min(Hash_set1[h],Hash_set2[h])
				checked1[h]=True
				checked2[h]=True

		for h in Hash_set2:
			if checked2[h]==False:
				SH+=min(Hash_set2[h],Hash_set1[h])
				checked2[h]=True
				checked1[h]=True
	...
	def Find_Plagiarism_Rate(self,SH,TH1,TH2):
		p_rate=(2*SH/(TH1+TH2))*100
...
if __name__=="__main__":
	...
	# Load the files/Documments
	file1=open(r'c:\users\saurabh\desktop\file1.txt')
	file2=open(r'c:\users\saurabh\desktop\file2.txt')

	obj=Plagirism_Detection(file1,file2)
